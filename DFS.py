...
class DFS:
    x_cord =  [-1,0,1,0] #[0,1,0,-1]
    y_cord = [0,-1,0,1] #[1,0,-1,0]
    path={}
    ...
    def dfsUtil(self):
        ...
        if self.isMazeSolved:
            self.getDFSPath(self.goal_x,self.goal_y)
    ...
    def solve(self):
        ...
        return Analyzer(self.matrix,self.pathCount,self.nodeExplored,len(self.stack),(new_time - old_time).total_seconds(),self.isMazeSolved,'DFS',self.probability,self.size)


    def getDFSPath(self,x,y):
